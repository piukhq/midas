...
def async_join_callback(scheme_slug, data, config, session):
    ...
    try:
        ...
        if error_code:
            if _is_error_retryable(error_code):
...
class JoinCallback(Resource):
    ...
    def post(self, scheme_slug, data, config, *, session: db.Session):
        thread_pool_executor.submit(async_join_callback, scheme_slug, data, config, session)
